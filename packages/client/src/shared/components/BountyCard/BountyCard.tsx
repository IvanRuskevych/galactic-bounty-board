...
import { BountyStatus, contextPage, UserRoles } from "~/shared/constants";
import { BountyCardActionButton } from "~/shared/ui";
import { useAuthStore, useStarWarsStore } from "~/store";
import { BountyCardProps } from "~/typings";
...
export const BountyCard = ({
	...
	const renderActions = () => {
		...
		if (context === contextPage.PUBLIC && bounty.status === BountyStatus.POSTED && isOwner) {
			return <BountyCardActionButton label="Posted by me" disabled />;
		}
	...
	return (
		...
		>
			...
			<CardActions sx={{ mt: "auto", justifyContent: "space-between" }}>{renderActions()}</CardActions>
		</Card>
	);
};
